...
11. **[11: Low-Stock Inventory Notifications (Backend)](./11-low-stock-inventory-notifications-backend.md)** — trigger a `low-stock` Pusher event on the existing `private-vendor-<shopId>` channel when an order drops a product's `inventoryCount` to/below a threshold. Reuses spec `06`'s channel/auth as-is. Pairs with the client repo's spec `13`. Implemented.
