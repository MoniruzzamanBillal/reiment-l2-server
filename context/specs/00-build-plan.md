...
6. **[06: Real-Time Order Notifications (Backend)](./06-realtime-notifications-backend.md)** — Socket.io server emitting `new-order`/`order-status-changed` events. Candidate "depth" feature (alternative: spec `07`). Pairs with the client repo's spec `10`. Plan written, awaiting user review/approval before implementation.
